    ...
    for( i = j = 0; i < nimages; i++ )
    {
        for( k = 0; k < 2; k++ )
        {
            ...
            if( displayCorners )
            {
                ...
                std::stringstream ss;
                ss<<"./save_img/corner_"<<corner_imgnum<<".jpg";
                corner_imgnum++;
...
// OR ELSE HARTLEY'S METHOD
    ...
    for( i = 0; i < nimages; i++ )
    {
        for( k = 0; k < 2; k++ )
        {
            Mat img = imread(goodImageList[i*2+k], 0), rimg, cimg,cnimg;
            cvtColor(img, cnimg, COLOR_GRAY2BGR);
            remap(img, rimg, rmap[k][0], rmap[k][1], INTER_LINEAR);
            cvtColor(rimg, cimg, COLOR_GRAY2BGR);
            Mat canvasPart = !isVerticalStereo ? canvas(Rect(w*k, 0, w, h)) : canvas(Rect(0, h*k, w, h));
            Mat norefPart=!isVerticalStereo ? noref(Rect(w*k, 0, w, h)) : noref(Rect(0, h*k, w, h));
            resize(cimg, canvasPart, canvasPart.size(), 0, 0, INTER_AREA);
            resize(cnimg, norefPart, norefPart.size(), 0,0, INTER_AREA);
            if( useCalibrated )
            {
                Rect vroi(cvRound(validRoi[k].x*sf), cvRound(validRoi[k].y*sf),
                          cvRound(validRoi[k].width*sf), cvRound(validRoi[k].height*sf));
                rectangle(canvasPart, vroi, Scalar(0,0,255), 3, 8);
                rectangle(norefPart, vroi, Scalar(0,0,255), 3, 8);
            }
        }
        ...
        if( !isVerticalStereo )
            for( j = 0; j < canvas.rows; j += 32 ){
                ...
                line(noref, Point(0, j), Point(noref.cols, j), Scalar(0, 255, 0), 1, 8);
            }

        else
            for( j = 0; j < canvas.cols; j += 32 ){
                line(canvas, Point(j, 0), Point(j, canvas.rows), Scalar(0, 255, 0), 1, 8);
                line(noref, Point(j, 0), Point(j, noref.rows), Scalar(0, 255, 0), 1, 8);
            }
        ...
        if( c == 27 || c == 'q' || c == 'Q' )
            ...
            imwrite("ref.jpg", canvas);
            imwrite("noref.jpg", noref);
        }
