...
cv::fisheye::initUndistortRectifyMap(M1, D1, R1, P1, img_right.size(), CV_16SC2, mapl1, mapl2);
    ...
    for (;;) {
        ...
        //waitKey();
        if(draw_rect)
            rectangle(hist_img_l, selection, cv::Scalar(g_rng.uniform(0, 255), g_rng.uniform(0, 255), g_rng.uniform(0, 255)));
        char key_s=waitKey(10);
        //
        
        //roi_l=hist_img_l(selection);

        //Mat roi_r;
        roi_r=hist_img_r(Rect(150, 70,380,310));
        roi_l=hist_img_l(Rect(150, 70,380,310));
        //Mat imgDisparity16S = Mat(roi_l.rows, roi_l.cols, CV_16S);
        //Mat imgDisparity8U = Mat(roi_l.rows, roi_l.cols, CV_8UC1);

        Mat imgDisparity16S = Mat(roi_l.rows, roi_l.cols, CV_16S);
        Mat imgDisparity8U = Mat(roi_l.rows, roi_l.cols, CV_8UC1);
        
		//sgbm->compute(roi_l, roi_r, imgDisparity16S);
        sgbm->compute(roi_l, roi_r, imgDisparity16S);
		minMaxLoc(imgDisparity16S, &minVal, &maxVal);
		imgDisparity16S.convertTo(imgDisparity8U, CV_8UC1, 255/(maxVal-minVal));

		//sgbm->compute(roi_l, roi_r, disp);
        //cout<<"histimg size:"<<hist_img_l.size()<<endl;
       // cout<<imgDisparity8U.size()<<endl;
        ...
        //    cv::imshow("right", img_right);
            sprintf(label, "(%d,%d)", current_point.x, current_point.y);
        putText(hist_img_l, label, current_point, FONT_HERSHEY_SIMPLEX, 1.0, Scalar(g_rng.uniform(0, 255), g_rng.uniform(0, 255), g_rng.uniform(0, 255)));
	    cv::imshow(name_window, hist_img_l);
            ...
            {
                num_dis++;
                sprintf(dis_name,"dis_%d.jpg",num_dis);
                sprintf(canv_name,"canv_%d.jpg", num_dis);
                imwrite(dis_name, imgDisparity8U);
                imwrite(canv_name, canvas);
