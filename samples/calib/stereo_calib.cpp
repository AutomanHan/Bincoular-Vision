...
StereoCalib(const vector<string>& imagelist, Size boardSize, float squareSize, bool displayCorners = false, bool useCalibrated=true, bool showRectified=true)
{
    ...
    double rms = fisheye::stereoCalibrate (
	        objectPoints,
		imagePoints[0],
		imagePoints[1],
		cameraMatrix[0], distCoeffs[0],
...
// OR ELSE HARTLEY'S METHOD
 ...
 // from the fundamental matrix
    ...
    for( i = 0; i < nimages; i++ )
    {
	   ...
	   // if (save_flag)
	    //{
		//sprintf(rect_img, "rmap%d.jpg",i*2+k);
		//save_flag = false;
	    	//imwrite(rect_img,rimg);
		//sprintf(rect_img, "src%d.jpg", i*2+k);
		//imwrite(rect_img, img);
	    //}
	...
	//sprintf(rect_img, "rectified%d.jpg", i);
	//imwrite(rect_img, canvas);
        char c = (char)waitKey();
        if( c == 27 || c == 'q' || c == 'Q' )
            break;
    }
